...
def ppo_loss(network: Network, minibatch, config: PPOConfig):
    ...
    current_log_probs = jnp.take_along_axis(log_probs, batch.actions[..., None], axis=-1).squeeze(
        -1
    )
    old_log_probs = jnp.take_along_axis(prev_log_probs, batch.actions[..., None], axis=-1).squeeze(
        -1
    )
...
def ppo_update(
    ...
    return update_network(network, optimizer, minibatches, ppo_loss, config)
