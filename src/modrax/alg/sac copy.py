...
class Actor(Network):
    def __init__(self, obs_shape: Shape, action_dim: int, config: SACNetworkConfig, rngs: nnx.Rngs):
    ...
    def get_action(self, x: Float[Array, "B D"], key: Key[Array, ""]):
        ...
        log_prob = jnp.sum(log_prob, axis=-1, keepdims=False)
...
class Critic(Network):
    def __init__(self, obs_shape: Shape, action_dim: int, config: SACNetworkConfig, rngs: nnx.Rngs):
        flat_input_size = math.prod(obs_shape) + action_dim
...
class SACNetwork(Network):
    def __init__(self, obs_shape: Shape, action_dim: int, config: SACNetworkConfig, rngs: nnx.Rngs):
        self.critic = Critic(obs_shape, action_dim, config, rngs)
        self.critic_target = nnx.clone(self.critic)
        self.actor = Actor(obs_shape, action_dim, config, rngs)
        self.log_alpha = LogAlpha(jnp.log(jnp.array(config.alpha)))

        self.running_norm = (
            RunningNorm(obs_shape, obs_shape, RunningNormConfig(), rngs=rngs)
...
def value_update(network, minibatch, config, key):
    def value_loss(critic, batch, key):
        ...
        next_action, next_action_log_prob, _, _ = network.actor.get_action(next_obs, key)
...
def policy_update(network, minibatch, config, key):
    def policy_loss(actor, batch, key):
        ...
        sample_act, log_pi, _, _ = actor.get_action(obs, key)
...
def alpha_update(network, minibatch, target_entropy, config, key):
    def alpha_loss(log_alpha, batch, key):
        obs = network.running_norm(batch.obs)
        _, log_pi, _, _ = network.actor.get_action(obs, key)
...
class SACAlg(Alg):
    def __init__(self, env: Env, alg_config: SACConfig, key, jit: bool = False):
        ...
        self.network = SACNetwork(
            env.obs_shape, env.action_dim, self.config.network_config, nnx.Rngs(key)
        )
        self.target_entropy = -0.5 * env.action_dim
        ...
        self.env_state, trajectories = self.rollout_fn(
            self.network.actor, self.env.step, self.env_state, num_steps, key
        )
    ...
    def make_loop_fn(self):
        def loop(state, key):
            ...
            env_state, trajectories = self.rollout_fn(
                network.actor, self.env.step, env_state, 1, rollout_key
            )
            transitions = jax.tree.map(lambda x: x.reshape(-1, *x.shape[2:]), trajectories)
