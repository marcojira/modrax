...
class SACNetwork(Network):
    def __init__(
        ...
        self.critic = Critic(obs_shape, action_dim, rngs=rngs)
        self.critic_target = nnx.clone(self.critic)
        self.actor = Actor(obs_shape, action_dim, rngs=rngs)
    ...
    def get_action(self, x: Float[Array, "B D"], key: Key[Array, ""]):
        ...
        # Log prob with action bound correction
        log_prob = -0.5 * ((x_t - mean) / std) ** 2 - 0.5 * jnp.log(2 * jnp.pi) - jnp.log(std)
        log_prob -= jnp.log(self.actor.action_scale * (1 - y_t**2) + 1e-6)
        log_prob = jnp.sum(log_prob, axis=-1, keepdims=True)
        mean = jnp.tanh(mean) * self.actor.action_scale + self.actor.action_bias
        return action, log_prob, mean, x_t
    ...
    def get_qs(self, obs, action):
        return self.critic.get_qs(obs, action)

    def get_target_qs(self, obs, action):
        return self.critic_target.get_qs(obs, action)
...
class SACAlg(Alg):
    ...
    def __call__(self, key: Key[Array, ""]):
        ...
        self.actor.train()
        batch, _, _ = self.buffer.sample(self.buffer_state, sample_key, self.config.minibatch_size)
        ...
        # # Policy loss
        # if self.iter % self.config.policy_frequency == 0:
        ...
        # if self.iter % self.config.target_network_frequency:
        #     state = nnx.state(self.critic)
        #     target_state = nnx.state(self.critic_target)
        #     new_target_state = jax.tree.map(
        #         lambda p, tp: self.config.tau * p + (1 - self.config.tau) * tp,
        #         state,
        #         target_state,
        #     )
        #     nnx.update(self.critic, new_target_state)
